...
```
    ...
    <matplotlib.image.AxesImage at 0x27671232fc8>
...
```
    ...
    C:\Users\micha\anaconda3\envs\tensorGPU2\lib\site-packages\ipykernel_launcher.py:23: DeprecationWarning: np.asscalar(a) is deprecated since NumPy v1.16, use a.item() instead
        ...
        for h in range(n_H):                   # loop over vertical axis of the output volume
            for w in range(n_W):               # loop over horizontal axis of the output volume
                for c in range(n_C):           # loop over the channels of the output volume
                    ...
                    vert_start = vert_start = h * stride
                    vert_end = vert_end = vert_start + f
                    horiz_start = w * stride
                    horiz_end = horiz_start + f
